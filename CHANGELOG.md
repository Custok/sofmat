...
## v202609252154rid (2026-09-25)
`/api/requests`: cada fila lleva ahora **`boot`** (arranque del proceso, UTC con milisegundos) y **`rid`** = `boot-id`, el nombre durable de una petición. `id` sigue siendo el contador por proceso que usa el panel, pero **vuelve a 1 en cada reinicio**: debian midió sobre 720 ficheros sellados (24 h, 4 reinicios) que el 67 % de los ids designaban más de una petición (`id 1` nombraba seis). Los informes que citen una petición deben usar `rid` o la hora (`t1_admit`); un `id` suelto es un puntero colgante. Sin cambio de comportamiento; dos pruebas (rid distinto entre dos vidas con el mismo id; etiqueta en UTC).

## v202609251929train (2026-09-25)
Transacción de entrenamiento (`internal/coordinator/train.go`): el runner del nodo que va a hacer fine-tuning **cerca** las instancias de producción que va a parar y las devuelve al servicio cuando acaban. Regla acordada por los cuatro devs (topic `fine-tuning-con-unsloth`): **quien lanzó la instancia la para y la arranca** — una unidad systemd la para su runner con `systemctl`, un motor lanzado por el coordinador se expulsa aquí si se pide (`parar`); el coordinador no mata por puerto lo que no es suyo, porque `Restart=always` lo resucitaría a los 5 s y el entrenamiento moriría por VRAM.
- `POST /api/train/begin {job, node, instancias[], deadline, drain_s, parar, nota}` (clave + POST): valida las claves contra `instances[]`, rechaza solapes (409), persiste la transacción en `train.local.json` (temporal + rename), **cerca**: el decode sale del reparto (una conversación pegada a él se mueve en su siguiente turno; con todos cercados la petición se rechaza con `ErrDecodeTraining` en vez de esperar) y el prefill sale del traspaso de KV (decode-directo); **drena** hasta `drain_s` (90 s por defecto, tope 600) y responde `drenado` + estado por instancia. `deadline` admite RFC3339 o `HH:MM` (hora local del coordinador, siguiente ocurrencia); vacío = +6 h.
- `POST /api/train/end {job, levantar, espera_s, forzar}`: opcionalmente levanta lo que el coordinador puede (preset con el mismo endpoint), espera `/health` hasta `espera_s` (120 s por defecto) y cierra si todo responde. Si algo sigue caído, **la transacción queda abierta en fase `remontando`** (cerca mantenida, vigilante armado) y responde 202; `forzar` cierra igual.
- `GET /api/train`: transacciones abiertas con la salud actual de cada instancia (sin clave, como `/api/status`).
- Vigilante cada minuto: cierra la transacción cuando las instancias vuelven; pasado el `deadline` con algo caído, intenta levantar lo que es suyo y **avisa una sola vez**: siempre al log, y por WhatsApp si `alerts.whatsapp_target` está en `config.local.json` (vía el contenedor de openclaw de este host; la confirmación es la línea `Sent via gateway`, nunca el rc).
- Panel: la tarjeta cercada muestra «en entrenamiento · job X · remonta antes de HH:MM» (punto de color acento, sin botón de expulsar); si el motor ya está parado, la tarjeta **no desaparece**, se sintetiza desde la config para que se vea por qué falta el decode y hasta cuándo. `/api/runtime` (stats del balanceador) lleva `training` por motor.
- Arranque: las transacciones persistidas se restauran y sus cercas se reaplican antes de aceptar tráfico.
- Seis pruebas nuevas: cerca del balanceador (nuevas conversaciones, pegadas, todos cercados), prefill fuera del traspaso, begin/end por HTTP con persistencia y conflictos, end con el motor caído (202 + fase remontando + aviso único + cierre forzado), ida y vuelta del fichero con reaplicación de cercas, formas del deadline.
